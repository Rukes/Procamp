...
export async function userRoutes(app: FastifyInstance) {
  ...
  app.get("/", { preHandler: guard }, async () => {
    return app.prisma.user.findMany({
      select: { id: true, name: true, email: true, isSuperAdmin: true, permissions: true, createdAt: true },
  ...
  app.put("/:id", { preHandler: guard }, async (request, reply) => {
    ...
    const body = request.body as { name?: string; email?: string; password?: string; isSuperAdmin?: boolean; permissions?: Record<string, boolean> };
